"""Maintainer CLI for the RSZ field-layout snapshot pair rsz_layout.py
manages (current + previous) -- see that module's "snapshot management"
section for the actual logic; this is a thin command-line wrapper around it,
plus the one thing an ordinary user's GUI never needs: baking a fresh
snapshot from a raw ~100MB community registry dump. See rsz_layout.py's
top-of-file docstring for why this snapshot exists and what it protects.
    ...
    python tools/bake_rsz_snapshot.py import <snapshot_file> --as current|previous [--half current|previous] [--label L] [--game-date D]
...
bake    Turns a raw rszmhwilds.json-style dump (typeIDHash -> {crc, name,
        ...
        project ships, and installs it as "current". --rotate archives the
        existing current as previous first -- another community fixer's
        README states the same rule plainly: "don't lose the previous one
        -- after a game update, today's current becomes tomorrow's
        previous." Skip --rotate only on the very first bake of a new
        registry generation.

list    Shows current and previous side by side: label, game_update_date
        (when that title update actually shipped -- distinct from
        baked_at, when this project processed it), source, how many typed
        classes.
...
import  Installs a snapshot someone else shares -- this project's own
        ...
        This is how a real "previous" registry gets acquired for a
        version this project never independently dumped -- borrowing one
        from someone who already has it is far cheaper than re-deriving
        it, and is exactly how this project's own TU5 "current" snapshot
        was obtained on 2026-08-08 (see CLAUDE.md #9).
...
def cmd_bake(args):
    ...
    if args.rotate:
        print(f"previous snapshot archived to {rsz_layout.PREVIOUS_PATH.name}"
              if rsz_layout.PREVIOUS_PATH.exists() else "(no prior current snapshot existed to archive)")
...
def cmd_import(args):
    try:
        ...
        sys.exit(str(exc))
    dest = rsz_layout.CURRENT_PATH if args.as_ == "current" else rsz_layout.PREVIOUS_PATH
    print(f"installed {meta['entry_count']} typed classes as {dest.name} (label: {meta['label']!r})")
...
def main():
    ...
    p_list = sub.add_parser("list", help="show current and previous snapshots")
    ...
    p_import.add_argument("--no-rotate", action="store_true", help="overwrite current without archiving it as previous first")
