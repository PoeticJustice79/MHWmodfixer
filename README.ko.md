...
아래 세 가지 위험한 옵션은 GUI에서 "⚠ 실험적 옵션" 그룹으로 함께 묶여 있고,
"게임에서 직접 확인하라"는 공통 경고 문구가 붙어 있음 — 셋 다 기본은 꺼져 있음.

**"안 고쳐진 부위 강제 수정" 체크박스**: 기본은 꺼져
...
**"셰이더 전환" 체크박스**: 기본은 꺼져 있음. 일부 셰이더는 게임 전체적으로
사실상 퇴역한 상태인데(현재는 `Base_Equip_Fur.mmtr` 하나만 해당 — 게임 전체를
뒤져봐도 이 셰이더를 실제로 쓰는 재질이 2개밖에 안 남음), 이런 셰이더로 만들어진
모드는 도너 매칭 자체가 안전하게 안 됨 — 남아있는 도너들이 전부 의미상 무관한
것들뿐이라서(실제 확인된 사례: "OVR Rogue - Bifrost"의 흰 재질/날개 소실 버그).
이 체크박스를 켜면 해당 재질을 그 셰이더의 실제 확인된 후속 셰이더
(`Base_Equip_Fur.mmtr` → `Base_Equip.mmtr`)로 다시 만들어줌. 두 가지 방식으로
검증됨: 한 모딩 커뮤니티의 실제 작가 수정본 17개랑 필드 단위로 비교(72개 재질
전부 100% 일치), 그리고 그중 하나("OVR Rogue - Bifrost")는 실제 인게임 테스트로
버그가 실제로 고쳐지는 것까지 확인함. **근데 이것과 무관한 파이프라인의 다른
모드("TiNE's Qipao Ver.R Remastered")에서는 착용하자마자 게임이 그대로
크래시나는 것도 확인됨** — 사전 정적 검사는 똑같이 다 통과했는데도. 즉 정적
검사를 다 통과해도, 실제로 검증된 그 특정 모드 풀 밖에서는 인게임 안전성이
보장 안 됨. 이 옵션으로 만든 결과물은 반드시 게임에서 직접 확인해야 함.
...
- 수정한 mdf2 외 나머지 항목은 **원본 압축 바이트를 그대로 재사용**(압축
  ...
  씀 (`pak_writer.py`).
- 모드 자체 pak 안에 번들된 `.pfb` 항목도 loose file `.pfb` 복구(아래 pfb
  섹션 참고)와 완전히 같은 안전한 방식으로 고친다 — 똑같은 헬퍼 함수를
  그대로 재사용: CRC만 낡은 경우 패치를 먼저 시도하고, 둘이 충분히
  비슷하면 도너 바이트로 통째 교체. 여기도 도너를 해시로 정확히 찾기
  때문에 경로/캐릭터 코드 치환 단계 자체가 필요 없다.
...
## 게임이 업데이트되면 일부 수정 기능이 잠깐 더 보수적으로 동작하는 이유

MHWmodfixer는 대부분의 수정 작업을 현재 설치된 게임 파일에서 직접 읽어와서
처리한다. 그래서 이 부분은 게임이 업데이트돼도 툴 쪽에서 별도로 손볼 필요
없이 자동으로 정확하게 동작한다.

딱 한 부분만 다르게 동작한다: 프리팹/캐릭터 데이터(`.pfb`/`.user`/`.scn`)
수정이 실제로 안전한지 검증하는 부분. 이 검증은 모드 자체의 데이터를,
현재 게임의 내부 데이터 구조를 미리 찍어둔 스냅샷(`tools/rsz_fields_mhwilds.json.gz`,
툴에 같이 들어있음)과 비교하는 방식으로 이뤄진다. 캡콤이 게임을 업데이트하면
이 스냅샷이 딱 그날부터 낡은 정보가 되어버릴 수 있는데 — 그리고 하필 그날부터
이런 종류의 수정이 필요한 모드들이 나타나기 시작한다. 이 틈에서 툴은 절대
추측하지 않는다. 검증이 안 되는 건 그냥 손대지 않고 그대로 둔다(이 기능이
생기기 전과 동일하게) — 안전해 보이지만 실제로는 아닌 수정을 적용하는
위험을 감수하지 않기 위해서다. 실제로 정확히 이 틈 때문에 발생한 크래시가
이 안전장치 자체가 생기게 된 계기였다 — 전체 경위는 `CLAUDE.md` 9번 항목
참고.

그래서 몬스터헌터 와일즈가 업데이트된 직후에는, 새 스냅샷이 만들어져서
다음 MHWmodfixer 릴리즈에 반영되기 전까지 이 특정 수정 기능만 평소보다
보수적으로 동작하는 짧은 공백이 생긴다. 재질/텍스처/경로 수정 등 나머지
기능은 게임 파일을 실시간으로 읽기 때문에 영향 없이 바로 정상 동작한다.

**이 공백을 새 MHWmodfixer 릴리즈가 나올 때까지 기다리지 않고 직접 메울
수도 있다.** 프로그램에서 **설정 → 개발자 옵션 → RSZ 스냅샷...** 을 열면
지금 뭐가 설치되어 있는지(라벨, 어떤 게임 업데이트를 나타내는지, 몇 개
타입을 커버하는지) 보여준다. 여기서:

- **GitHub에서 최신 데이터 확인...** 을 누르면 원본 출처
  ([github.com/seifhassine/REasy](https://github.com/seifhassine/REasy),
  ~100MB — 다운로드 전에 확인 창이 뜬다)에서 바로 최신 타입 레지스트리를
  받아서 설치하거나,
- 누군가 이미 가공해서 공유해준 작은 스냅샷 파일이 있다면(넥서스 댓글,
  디스코드 등) **스냅샷 가져오기...** 로 그 파일을 선택하면 된다.

둘 다 재시작 없이 바로 적용되고, 새 "현재판"을 설치하면 기존에 현재판이던
것은 자동으로 날짜가 붙어서 보관된다(이 프로그램이 현재판으로 설치했던
스냅샷은 전부 계속 남아있다) — 그냥 사라지지 않으므로 파괴적인 동작이
아니다. 가능하면 실제 설치된 게임 파일 중 하나로 자동 검증도 하는데,
검증이 안 되더라도(원본 데이터 자체의 알려진 한계 때문이지 뭔가 잘못됐다는
뜻이 아님) 그냥 안 된다고 솔직하게 알려준다 — 확인 안 된 걸 확인됐다고
속이지 않는다.

유지보수용 참고: `tools/bake_rsz_snapshot.py`(GUI 다이얼로그와 같은
`rsz_layout.py`의 함수들을 그대로 쓰는 CLI 도구)가 이 스냅샷을 관리한다
— 타이틀 업데이트 이후에는 `bake --rotate`로 기존에 현재판이던 것을
`tools/rsz_archive/`에 보관한 다음 새로 굽고, `list`로 현재판 + 지금까지
보관된 모든 스냅샷을 확인하고, `import`로 다른 사람이 공유해준 스냅샷을
현재판으로 바로 설치하거나 보관함에만 넣을 수 있다. 자세한 내용은 해당 파일의
모듈 docstring 참고.
