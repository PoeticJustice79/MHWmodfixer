...
둘 다 재시작 없이 바로 적용되고, 새 "현재판"을 설치하면 기존 것은 자동으로
"이전판"으로 보존되므로 파괴적인 동작이 아니다. 가능하면 실제 설치된 게임
파일 중 하나로 자동 검증도 하는데, 검증이 안 되더라도(원본 데이터 자체의
알려진 한계 때문이지 뭔가 잘못됐다는 뜻이 아님) 그냥 안 된다고 솔직하게
알려준다 — 확인 안 된 걸 확인됐다고 속이지 않는다.

유지보수용 참고: `tools/bake_rsz_snapshot.py`(GUI 다이얼로그와 같은
`rsz_layout.py`의 함수들을 그대로 쓰는 CLI 도구)가 이 스냅샷을 관리한다
— 타이틀 업데이트 이후에는 `bake --rotate`로 기존 스냅샷을 버리지 않고
보존하면서(오늘의 "현재판"이 내일의 "이전판"이 되는 방식) 새로 굽고,
`list`로 지금 번들된 스냅샷을 확인하고, `import`로 다른 사람이 공유해준
스냅샷을 직접 만들지 않고 바로 설치할 수 있다. 자세한 내용은 해당 파일의
모듈 docstring 참고.
