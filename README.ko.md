...
수도 있다.** 누군가 새 스냅샷 파일을 공유하면(넥서스 댓글, 디스코드 등 —
관리자는 타이틀 업데이트 직후 몇 분 안에 스냅샷 하나만 구워서 바로 공유할
수 있고, 정식 빌드가 나오는 것보다 훨씬 빠르다) 프로그램에서
**설정 → 개발자 옵션 → RSZ 스냅샷...** 을 열고 **스냅샷 가져오기...** 를
눌러 해당 파일을 선택하면 된다. 설치 전후로 지금 뭐가 들어있는지(라벨,
어떤 게임 업데이트를 나타내는지, 몇 개 타입을 커버하는지) 보여주고,
재시작 없이 바로 적용된다. 새 "현재판"을 설치하면 기존 것은 자동으로
"이전판"으로 보존되므로 파괴적인 동작이 아니다. 걱정되면 "이전판"으로
설치해도 되는데, 이건 순수 참고용이라 실제 수정 동작에는 영향을 주지
않는다.
