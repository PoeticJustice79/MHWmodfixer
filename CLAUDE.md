...
### 47. Artian tier data cross-verified via a real REFramework mod's own logic (ArtianEditor, 2026-08-13) -- confirms the Gogmazios-final names are already complete, and explains the plain-tier gap
...
### 48. it10/03/* was never weapons at all -- it's the Kinsect roster, now named in 4 languages via Kiranico cross-reference (2026-08-13)
...
### 49. sid=10's remaining 162 unnamed slots explained -- mostly orphaned files, not a data gap; and a real mislabeling in CONFIRMED_MANUAL_NAMES found (2026-08-13)
...
### 50. Password-protected archive support (2026-08-14)
...
### 53. Item #52's weapon-gimmick investigation resolved -- confirmed via live A/B testing, not just theory (2026-08-14)

Direct continuation of item #52 (GS_Artian_LVL5/"Varianza", retargeted `it00/00/0002` -> `it00/00/0007`, sheathed/drawn glow stuck permanently on). User pushed to keep investigating with live REFramework data rather than accept the earlier static-analysis conclusion.
...
`mhwmodfixer_gs_artian_lvl5_parts_fix.lua` and its `FORCE_STATE` override are left in place (set back to `nil`, i.e. inert/no-op by default) in case a future session wants to revisit a DIFFERENT weapon mod that turns out to have this exact same "extra piece"-shaped gap (mirroring item #51's armor finding) but whose mesh genuinely does contain both states -- the working live-inspection technique and the two-tier ModelPartsManager2/via.render.Mesh API are now a proven, reusable path to that answer without needing another multi-hour live debugging session from scratch.

**CORRECTION (2026-08-14, same day): the conclusion above is WRONG.** The `FORCE_STATE` A/B test that led to "the mesh doesn't contain two real shapes" was run EXCLUSIVELY against the weapon equipped at the RETARGETED slot (`ace_fix.lua`'s own `TARGET_NAME_SUBSTRING = "0007"` gate ensured this) -- it never once tested the mod at its own native slot. Isolated via a clean, controlled real in-game test: built two zips, both using the IDENTICAL repaired material (`AnimEmit_Blend` and the other 12 newly-added shader properties from #52's own material diff all present, at the donor's own default values, completely unmodified), differing ONLY in whether the file was retargeted to `it00/00/0007` or left at the mod's own native `it00/00/0002`.
- **Native slot (`it00/00/0002`), repair only, no retargeting: transform toggles correctly** (sheathed/drawn glow switches as expected) -- confirmed directly by the user. This proves the repaired material (all 13 new properties, `AnimEmit_Blend=0.0` included) is NOT the cause of anything -- it was never broken to begin with.
- A separate, riskier experiment (forcing `AnimEmit_Blend` to `1.0` instead of the donor's `0.0`, same native slot) caused the game to hang/black-screen at boot -- a real, useful data point (confirms this property isn't an inert no-op the way a random unused field would be -- forcing it to an unexpected value has a genuine engine-level consequence, likely because `1.0` signals "read from a bound animation-curve resource" that this weapon's pfb never actually wires up) but NOT the mechanism behind the original bug, since the donor's own default (`0.0`) already works fine at the native slot. This value should not be experimented with further -- treat `0.0` (whatever the donor's own current material carries) as the only confirmed-safe value, matching this project's own established `_transplant_reshaped()` lesson (#18) about not pushing forward with further guesses once a wrong value has already demonstrated an undiagnosable boot-time failure.

**Real, now-confirmed root cause: retargeting alone breaks it, independent of the material fix** -- which points squarely back at #52's own original finding, now with the material-fix hypothesis eliminated as a competing explanation: slot `0002`'s real vanilla pfb references a dedicated `Motion/Player/WeaponGimmick/Wp00_01/Wp00_01Gimmick.motbank`; slot `0007`'s real vanilla pfb uses the generic, type-wide `Wp00/Wp00Gimmick.motbank`. This mod ships no pfb of its own, so retargeting it to `0007` makes it inherit `0007`'s vanilla pfb -- which has no dedicated gimmick wiring to trigger the transform at all, leaving the mesh/material sitting at whatever static state it initializes to (the "glow on" baseline), regardless of real draw/sheathe state. The mesh itself is fully correct and DOES contain two real states (proven by the native-slot test above); nothing about it or the material needs fixing.

**Practical implication for this mod (and any mod like it)**: the sheathed/drawn transform glow can only work correctly at the mod's own NATIVE slot. Retargeting to a different slot cannot currently preserve this effect with tools available to this project -- attaching the source slot's dedicated gimmick-bank reference to the target's pfb was already attempted (a same-length UTF-16LE string substitution failed outright because the two bank path strings are different lengths; a full custom-pfb rebuild using the source's real vanilla pfb with the id substituted made the weapon fully invisible in-game, cause never diagnosed, abandoned -- see the "custom pfb" experiment earlier in this same investigation). This is a real, confirmed capability gap, not a guess -- if a future session wants to revisit it, the starting point should be figuring out why the custom-pfb approach caused total invisibility (a different failure mode than anything else this project's pfb tooling has hit), not re-attempting the same string substitution.

`mhwmodfixer_gs_artian_lvl5_parts_fix.lua` (the ModelPartsManager2 companion script) is now understood to be irrelevant to this specific bug -- parts 1/2 are a real toggle, but not the mechanism behind this weapon's transform (the real mechanism is pfb/gimmick-bank-animation-driven, at a level this project's Lua companion-script approach never actually needed to intervene in). Left in place, inert, in case a future investigation on a different mod finds an actual use for it.

### 54. Weapon retargeting now warns when a source slot has a dedicated WeaponGimmick bank the target doesn't share (2026-08-14)

Turned #52/#53's finding into a real, shipped warning, the same "detect and warn, don't silently drop" pattern #51 already established for armor's extra-piece gap.
...
**`weapon_retarget.py`**: `TargetWeaponCandidate` gained `loses_gimmick: bool`. `find_compatible_weapon_targets()` sets it whenever the SOURCE slot is `special_gimmick` and a given target candidate's own `gimmick_bank` doesn't match the source's exactly -- true for essentially every other slot in practice (a dedicated gimmick bank is normally unique to its own weapon), which is the correct, honest signal: retargeting AWAY from a special-gimmick slot to basically anywhere else loses that slot's animation-driven behavior (transform glow, etc.), regardless of which target is picked. This is INFORMATIONAL only, never blocking (unlike the `refused` grade for a mismatched bundled pfb) -- most mods sitting at a special-gimmick slot are ordinary reskins that never used the effect at all, so warning unconditionally rather than gating generation is the right call, mirroring #51's own reasoning.

**GUI**: new i18n key `note_weapon_loses_gimmick` (all 5 languages from the start, per #39's own standing lesson), appended to the existing `_weapon_note_for()` helper in `gui.py` alongside the physics-mismatch note (both notes concatenate when both apply) -- both call sites (initial candidate render and the language-switch `refresh_texts()` copy) already funnel through this one helper, so no duplicate wiring was needed.

Verified: re-baked `weapon_slots.json.gz` against the live game (622 entries, 102 flagged), re-merged weapon names immediately after per the baker's own documented pitfall (names would otherwise be silently wiped), confirmed `it00/00/0002`/`0007` still resolve to their real names ("Varianza"/"Firetrail Blaisdell") post-merge. `find_compatible_weapon_targets()` on a synthetic `it00/00/0002` source (mesh+mdf2 only, matching GS_Artian_LVL5's real shape) correctly flags `loses_gimmick=True` on `it00/00/0007` (and in fact on all 31 real candidates, as expected -- the dedicated bank is unique to 0002 itself). Headless GUI smoke test (the established `TkinterDnD.Tk()` + real mainloop pattern) confirms the weapon retarget dialog still opens without exception. No other module touched -- the core repair pipeline (`auto_fix.py`/`slot_merge.py`/`pfb_fix.py`/armor's `slot_retarget.py`) is unaffected.
