...
**Not yet fixed or even fully confirmed** -- would need direct pfb-level investigation (comparing the two slots' real vanilla pfbs for how they reference/gate submesh state) that wasn't done this session. Unlike item #51, there's no simple static signal (like "pieces_shipped > 5") to key an automatic warning off here, so no code change was made. Flagged for a future session if this recurs: the concrete next step is diffing `0002` and `0007`'s real vanilla pfb RSZ instances for any state/visibility-gating component the two slots might have differently.
