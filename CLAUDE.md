...
### 46. subid=03 (Kinsects) solved with real official 5-language names -- subid=01 investigated further, confirmed genuinely unsolvable with current sources (2026-08-12)

Direct follow-up to #45's "not yet done" flag. Two separate investigations, one closed, one confirmed still open.

**Kinsects (it10/03/<iid>, InsectGlaive's "RodInsect" equipment slot) -- SOLVED, all 21 real official names in all 5 languages.** subid=03 has zero rows in `WeaponData.cData` (same as subid=01/99, confirmed repeatedly) because Kinsects were never part of that table to begin with -- they have their own separate data pair, found via `LartTyler/mhdb-wilds-data`'s config.toml: `Common/Equip/RodInsectRecipeData.user.3` (crafting-cost rows only -- `_Index`/`_ID`/`_KeyItemId`/`_KeyEnemyId`/`_PrevID`, no `_ModelId` and no `_Name` GUID field, so it can't be linked to a msg entry or a file-path iid the same direct way `WeaponData.cData` is) and `Excel_Equip/RodInsect.msg.23` (real per-kinsect text, all 5 languages -- confirmed msg entry names are literally `RodInsect_<the cData row's own _ID value>`, e.g. `_ID=1190390272` <-> `RodInsect_1190390272`, but that's still not a path to the file iid).

The actual bridge used: `tools/community_mhws_weapon_zh_cn.csv` (#45) already supplied zh_cn names for all 21 real it10/03/<iid> slots. Cross-checking those exact zh_cn strings against `RodInsect.msg.23`'s own zh_cn column found a **21/21 (100%) exact match, zero ambiguity** -- proof both sources describe the identical 21 kinsects, so the msg entry that matches by zh_cn text is the verified full-language source for that same iid. New `resolve_rodinsect_names()` in `bake_weapon_names.py` implements exactly this (msg parse -> keyed by zh_cn text -> matched against each existing subid=03 entry's already-baked zh_cn name), applied as a new merge pass in `main()`. Verified: all 21 `it10/03/*` entries now carry real `names: {ko/en/ja/zh_tw/zh_cn}` (e.g. `it10/03/0000` = 말드론/Mauldrone/マルドローン/綠色金龜子/绿金龟), `weapon_slots.json.gz` entry count unchanged (674, nothing added/removed, only existing zh_cn-only entries upgraded), `weapon_retarget.py` still imports cleanly. Applied as a standalone patch on top of the already-baked file (not a full `main()` re-run) specifically to avoid the documented "re-running the static-source path silently wipes the richer live-dump-sourced names" trap from #44's own notes -- a full re-bake would have downgraded subid=10/100 coverage back to the weaker static-only numbers.

**subid=01 (basic Iron/Bone-tier weapons, 32 entries across 13 of 14 types) -- investigated further, still confirmed unsolvable.** Checked two more candidate data tables directly against the live game, neither panned out: `WeaponSeriesData.user.3` (`app.user_data.WeaponSeriesData.cData`, 47 rows -- per-SERIES/tree data with its own `_Name` Guid field, but this links to the tree's overall name, not any individual tier/model, and subid=01 items were already confirmed to have zero presence in the crafting-tree table they'd need to belong to for this to apply) and `Player/EnumMaker/WeaponSeries.user.3` (an enum-only table, no name data at all). No `RodInsectData`-style sibling table exists for subid=01 the way `RodInsectRecipeData`+`RodInsect.msg` existed for Kinsects -- every plausible path variant tried (`RodInsectData`, `InsectData`, etc., see script history) returned not-found.

Given the item names themselves (Iron Sword/Bone Blade/Hunter's Bow-style, per the zh_cn CSV -- "铁剑"/"骨刃"/"猎弓") and the total absence from the normal crafting-tree table, the leading (still unconfirmed) theory is these are hardcoded STARTING/tutorial-gear items handed to a new hunter rather than normal craftable tree entries -- MH games traditionally have exactly this kind of non-craftable starter weapon. If true, their names would live in a save-data/new-game-initialization table this project has no reason to have looked at yet, not an equip-data table. Not pursued further this session (diminishing returns without a concrete new lead) -- subid=01 stays zh_cn-only, matching this project's "don't guess when the evidence runs out" discipline. If a future session wants to close this, the concrete next step is searching for a "starting equipment"/"new game" or "tutorial" data file via the same `LartTyler/mhdb-wilds-data` config.toml approach that found both the WeaponData and RodInsect pairs, rather than continuing to guess plausible `Common/Equip/*.user.3` file names one at a time.

subid=99 (8 entries, no names in ANY language including zh_cn -- the community CSV never covered it, `_COMMUNITY_TRUSTED_SIDS` is `{"00","01","03"}` only) was also surfaced this session but is a different, smaller, genuinely-zero-data gap -- not investigated further, not what the user was pointing at (they were specifically reacting to weapon names showing Chinese-ONLY, i.e. subid=01/03 which already had partial coverage, not subid=99 which has none at all).

### 47. The subid=01 zh_cn names from #45 were WRONG, not just incomplete -- real bug found while chasing the "new game init" lead, fixed by deletion (2026-08-12)

Continuing #46's "search a new-game-initialization/starting-equipment table" instruction: checked `WeaponSeriesData`/`WeaponTree`/`OuterWeaponData` (a real, previously-unknown "Layered Weapon" cosmetic catalog -- referenced only existing subid=00/10/100 models, zero subid=01) /`OtWeaponData` (Otomo/Palico buddy weapons, a wholly separate equipment category, unrelated) /`WeaponRecipeData` (crafting-cost only, same shape as `RodInsectRecipeData`, no model/name link) directly against the live game via `LartTyler/mhdb-wilds-data`'s full game file-list (`tools/ree-pak-gui/.../MHWs_STM_Release.list`, 337k real paths -- a genuinely new, useful resource for this project, letting candidate paths be found by real listing instead of blind guessing). None of these tables reference subid=01 either -- the "new game init" lead, while a reasonable theory, did not pan out with any table this project or the community datamining project's own config.toml knows about.

**While checking their pre-built `output/merged/weapons/GreatSword.json`** (a mature, independently-built database from the SAME community project, generated from real `WeaponData.cData`, not a guess) **for one last cross-check, found something much more important than the original question**: their "game_id=25" entry, named "Ravager Blade"/"藏钩大剑"/"블레이즈블레이드", with real crafting-tree metadata (rarity 8, previous_id, branches, column/row) -- **exactly matching `it00/01/0006`'s already-shipped zh_cn name from #45's own community CSV**. But re-verifying THIS specific weapon's real `_ModelId` directly against the live game (via the project's own established, trusted `_extract_index_model_id()`) found it decodes to **`model_id=10006` -> subid=10, iid=6 -- NOT subid=01 at all.**

**Systematically re-checked all 31 of the CSV's "01_<iid>" rows the same way** (their claimed `01_<iid>` vs this project's own already-resolved `it<code>/10/<iid>` real official name, tier-numeral suffix ignored): **31/31 (100%) match.** Every single "01_<iid>" row in the third-party CSV is actually describing `it<code>/10/<iid>` in this project's own `model_id`-based convention -- not subid=01 at all -- and **every one of those 31 real subid=10 slots already has a real, better, already-baked official multi-language name** from this project's own independent live-game resolution (e.g. `LONG_SWORD,01_0004,破坏者大剑` is really `it00/10/0004` = "破坏者大剑Ⅰ", already known).

**Root cause of the original mistake (#45)**: `_COMMUNITY_TRUSTED_SIDS = {"00","01","03"}` was set on the reasoning "subid=00 matched 340/340, so 01/03 are probably fine too" -- but that 340/340 match rate never actually tested whether the source's own `sid` value used the same divisor as this project's `model_id // 1000` convention, because every subid=00 `model_id` is under 1000 and looks IDENTICAL under any larger divisor too (e.g. `model_id // 1000` and `model_id // 10000` both give `0` for `model_id=4`). The source's own "01" bucket most likely corresponds to `model_id // 10000 == 1` (i.e. this project's subid=10), not `model_id // 1000 == 1` (subid=01) -- a genuine, previously-unnoticed axis mismatch between the two projects' numbering conventions. This also retroactively fully explains #45's own already-recorded "subid=10 matched only 1/23" oddity: that check was comparing against the source's "10" label, when the real subid=10 matches were sitting under the source's "01" label the whole time.

**subid=03 (Kinsects) is unaffected by this bug** -- #46's fix never trusted the source's own id-bucket label at all; it verified by matching the exact zh_cn TEXT against `RodInsect.msg.23`'s real official content (21/21 exact match), a verification method that doesn't depend on knowing what the source's internal numbering scheme means. Re-confirmed no change needed there.

**Fix**: `_COMMUNITY_TRUSTED_SIDS` narrowed to `{"00", "03"}` (removed "01"); `load_community_zh_cn_names()`'s module-level comment rewritten to record this whole investigation for the next person. Since every one of the 31 mislabeled rows' REAL slot (subid=10) already has a proper name, there was nothing to re-home them TO -- the fix is a pure deletion: all 31 (32 keys touched, one already had no name) `it*/01/*` entries' wrongly-attached `names` field removed directly from `weapon_slots.json.gz` (a surgical patch on the already-baked file, not a full `main()` re-run, for the same reason #46 used one -- avoids re-triggering the weaker static-source path and downgrading the already-good live-dump data). Verified: `674` total entries unchanged, `0` subid=01 entries carry a `names` field anymore, `weapon_retarget.py` still imports cleanly.

**Net result: subid=01 is back to fully unresolved** (matching #46's original, correct "genuinely unsolvable with current sources" conclusion) **-- worse than #46 realized at the time, since the data it thought was "at least zh_cn-only" was actually wrong information being shown to users, not just incomplete information.** A wrong name is worse than no name, per this project's own standing "never guess" discipline -- this is exactly that principle being violated by accident (trusting an unverified assumption about a third-party source's own id scheme) and then caught and corrected once real evidence became available. If a future session finds a genuine subid=01 name source, verify it by TEXT content against independently-known data (the same method that kept subid=03 safe), never by trusting an id-bucket label match alone.

**Merge note (2026-08-14): the items below (originally numbered #46-#53) were written by a second, parallel session that started from the same base this project was at before #46/#47 above were pushed, so it independently re-numbered from #46 too and never saw the subid=01 correction above. See #56 at the very end of this run for the reconciliation.**

### 48. Shader migration's brand-new-slot donor bleed widened past "mask"-named slots -- Detail_ALBD_*/Detail_NRRH_* channels hit the same defect (2026-08-13)
...
### 49. Artian tier data cross-verified via a real REFramework mod's own logic (ArtianEditor, 2026-08-13) -- confirms the Gogmazios-final names are already complete, and explains the plain-tier gap

The user pointed at a real Nexus mod, "Artian Editor" (a REFramework Lua tool for respec'ing owned Artian weapons), noting it has "all" Artian + Gogmazios-final data. Read its Lua source directly (`ArtianEditor\reframework\autorun\artian_editor.lua`) rather than treating it as a black box -- it reveals the exact mechanism: `app.ArtianUtil.isArtianWeapon(app.user_data.WeaponData.cData)` is a real native flag for "is this row a valid Artian crafting base", and Gogmazios-final variants are detected by **duplicate localized names within the same weapon type's table** -- three rows sharing one name are the Attack/Affinity/Attribute-focus variants (told apart by `_Critical`'s sign: `<0`=Attack, `>0`=Affinity, `==0`=Attribute).

Wrote a one-shot diagnostic Lua (`mhwmodfixer_artian_dump.lua`, dropped in `reframework/autorun`, deleted after use, not part of this repo) mirroring that exact logic across all 14 weapon types, using the SAME live singleton (`VariousDataManager._Setting._EquipDatas._Weapon<Key>._Values`) #42's live-dump work already reads. **First attempt had a real bug**: read `row[<Key>]` (e.g. `row._Hammer`) as if it were the file-path-relevant model id -- but that field is actually a separate `app.WeaponDef.<Key>Id` enum value (used elsewhere for `FreeVal1`/save-data bookkeeping), a totally different numbering scheme from `_ModelId`. Caught immediately by cross-referencing against `weapon_slots.json.gz` and finding every decoded key came back with no existing table entry at all. Fixed by reading `row._ModelId` directly (the field this project's own `bake_weapon_names.py` has always used) and re-dumped via REFramework's live "Reset Scripts" (no game restart needed).

**Result, cross-checked against `weapon_slots.json.gz` for all 14 types:**
- **Every single Gogmazios-final-tier model (the duplicate-name trio) decodes to a subid=00 key that ALREADY has `has_mdf2=True` and an ALREADY-correct name in the table** (e.g. it04/00/0018 = "금계의 데스반켈", matching the live dump's `gogma_group_name` exactly) -- these were resolved by #42's general live-dump name pipeline already, with zero manual confirmation needed. This is a clean, independent cross-validation of that existing data, not new information -- worth recording as confirmation, not a code change.
- **The plain (non-Gogma) Artian tiers I/II/III share ONE `_ModelId` per weapon type, and that id decodes into the subid=100 band with NO discoverable file** -- e.g. it04's "아티어해머Ⅰ"/"Ⅱ"/"모토반켈" (three separate WeaponData.cData rows, three separate names) all point at the identical `_ModelId=100002`, which has zero mesh/mdf2/pfb anywhere in the game per this project's own existence scan. This is the exact same gap already flagged as unsolved in item #44 ("no discoverable mesh file anywhere under art/model/item/<code>/100/<iid>/") -- now explained rather than just observed: tiers I/II/III of the plain branch are very likely NOT geometrically distinct 3D assets at all (same visual model regardless of tier), which is why no per-tier file was ever findable.

**This also explains, rather than contradicts, the earlier user-confirmed `CONFIRMED_MANUAL_NAMES` entries** (`it04/00/0012`="모토반켈", `it04/10/0002`="아티어해머 (Ⅰ/Ⅱ 공용)", `it00/00/0002`="발리안차" -- see [[weapon_artian_tier_mapping]]): these do NOT match the canonical `_ModelId` this session traced (which has no file at all), but they ARE real, file-backed slots (`has_mdf2=True`, matching Chain2/ChainSetting physics) that a real downloaded reskin mod visually renders as those exact appearances when equipped. For THIS project's actual purpose (finding a real, retargetable file slot a mod can safely point at), the empirically-mod-tested slot is the practically correct answer regardless of what the abstract catalog data would consider canonical -- left completely unchanged, no correction needed.

**Net outcome**: no code changes. Confirms the Gogmazios-final tier is fully solved project-wide (14/14 types) with no remaining gap, and gives a real, evidence-based explanation for why the plain-tier gap can't be closed the same way -- it isn't a missing file this project failed to find, there is very likely no separate file to find at all.

### 50. it10/03/* was never weapons at all -- it's the Kinsect roster, now named in 4 languages via Kiranico cross-reference (2026-08-13)

While reviewing the sid=01/03 community-CSV names from item #45 for multi-language expansion, direct inspection of the 21 `it10/03/*` entries' zh_cn text (species-sounding names like "绿金龟"/Green Scarab, "原初锹形虫"/Primal Stag Beetle) revealed they aren't weapon names at all -- Insect Glaive's Kinsect (bug companion) roster shares `it10`'s file-numbering convention for a completely different item category. `it10/01/*` (2 entries) really are weapons; only `it10/03/*` specifically is Kinsects.

Separately investigated the remaining 32 real weapon entries (sid=01 across all other types) and found they're very likely genuine CUT CONTENT: they follow the classic "Iron/Bone + weapon type" naming convention used consistently across the whole 20-year MH series (confirmed via a live web search finding a real "Iron Katana I" -- Long Sword -- and "Hunter's Bow" -- Bow, both matching this project's own zh_cn text exactly), but **Wilds replaced that entire opening tier with a new "Hope"-branded one** (confirmed: every fetched Kiranico weapon-tree page starts with "Hope Blade I-V", no Iron/Bone tier at all) -- meaning these 32 assets predate that redesign and were never shipped with any official Wilds-specific localized text to find. Presented this distinction to the user directly; per their explicit call, these 32 stay zh_cn-only (no invented convention-based translation merged in) while the 21 real Kinsect names got the full treatment below.

**Kinsect names sourced from Kiranico's own database** (`mhwilds.kiranico.com/data/kinsects`), fetched in en/ko/ja/zh(-simplified) -- real per-item text, not machine translation. Kiranico's internal listing order does NOT match this project's own iid-ascending order, so position correspondence between the two had to be established before trusting anything: cross-referenced 5 independent semantic anchor points (the same position in all 4 fetched pages), each confirmed both by matching POSITION and matching MEANING independently -- e.g. "Pseudocath"/"시나토모도키"/"シナトモドキ" = "Cynato Mimic" at position 11 in every language, "OMG"/"꼬메가"/"プチオメガ" = "mini/petit Omega" at position 18. All 5 anchors agreed, confirming the 4 pages share one consistent underlying order, which was then used to map all 21 entries with confidence. zh_tw intentionally left out -- checked several URL path conventions on Kiranico (`/zh-tw/`, `/cht/`, `/hant/`, `/tw/`) and none returned a working Traditional Chinese locale for this game; `weapon_label()`'s existing English fallback covers the gap rather than guessing a Simplified→Traditional conversion that could silently pick the wrong regional terminology.

Added as 21 new entries in `tools/bake_weapon_names.py`'s `CONFIRMED_MANUAL_NAMES` (now 24 total, up from 3) via `.update({...})` right after the existing Artian block. Re-ran the baker (`python tools/bake_weapon_names.py <game_dir>`) -- verified "applied 24 manually-confirmed name(s)" in the output (3 pre-existing + 21 new), zero regression on the rest of the table (674 total entries unchanged, sid=100's 52/52 and sid=10's 29/29 name coverage both preserved from the prior live-dump bake).

### 51. sid=10's remaining 162 unnamed slots explained -- mostly orphaned files, not a data gap; and a real mislabeling in CONFIRMED_MANUAL_NAMES found (2026-08-13)

User asked whether the 162 still-unnamed sid=10 slots (out of 191 total, 29 named at the time) could be resolved the same way item #48 closed the Kinsect gap. Investigated by reading `_extract_index_model_id()`/`_load_msg_names()`'s RAW (pre-dedup) output directly per weapon type, rather than guessing.

**Answer: no, and for a good reason -- there's nothing there to find.** Across all 14 types, only **31 real `_ModelId` values exist in the whole sid=10 band** (2-3 per type; `weapon_slots.json.gz`'s existence-scan created 191 table entries because that many MESH FILES exist under the sid=10 path convention, but only 31 of those files are ever referenced by an actual `WeaponData.cData` row). The other ~160 are genuinely orphaned assets -- the exact same "file exists, nothing in the current game points at it" pattern already confirmed for sid=01 (item #50: pre-"Hope tier" cut content). Confirmed this is real, not a resolver bug, by direct inspection: e.g. it04 (Hammer) has real weapon data ONLY at model_id 10000 and 10003 -- nothing at 10001, 10002, 10007-10015 despite mesh files existing there.

**A second, more interesting finding**: each of those 31 real model_ids isn't just "one name" -- it's shared by multiple tier rows the SAME way item #49's Artian investigation found. E.g. it04's model_id=10000 has FOUR real localized names sharing one file (Iron Hammer I/II/III, then a unique final name "Iron Devil"/"귀철"), and model_id=10003 has FIVE (Bone Bludgeon I-IV, then "Heavy Bone Bludgeon"). `resolve_names()`'s existing "keep lowest msg index" dedup already handles this correctly (same mechanism used everywhere else in this pipeline) -- it just means the single name currently shown for that slot (e.g. "Iron Hammer I") is only the FIRST of several real display names sharing that file, not a data gap or defect.

**Real mislabeling found and left as-is per user's own call ("괜찮아")**: `CONFIRMED_MANUAL_NAMES`'s `it04/10/0002` entry ("아티어해머 (Ⅰ/Ⅱ 공용)"/"Artian Hammer (I/II shared)") sits at a model_id (10002) that this fresh data confirms has **zero real WeaponData.cData rows at all** -- it's one of the ~160 orphaned files, sitting between the real Iron (10000) and Bone (10003) branches purely by numeric coincidence. This retroactively clarifies (doesn't newly break) item #49's own finding that this label was never the game's own canonical text for that slot -- what item #47 didn't establish is that the slot ISN'T EVEN part of the Iron/Bone tree it appears to sit inside. The label stays factually defensible under its own existing docstring ("a real, file-backed retargeting slot a mod happens to use for that visual appearance, NOT the game's own canonical id") -- flagged to the user as worth tightening for clarity in a future session, but not changed now.

Net: no code change this entry -- 162/191 unnamed sid=10 slots is very likely the FINAL correct count (nothing left to resolve there), not a pending gap.

### 52. Password-protected archive support (2026-08-14)

Real need, not hypothetical: a Mangie mod ("Echo") turned out to be an entirely AES-encrypted zip (every entry, `compress_type=99`, not just one file) -- confirmed stdlib `zipfile` can't read it under any circumstance: with no password it raises `RuntimeError` ("encrypted, password required"), and even WITH the correct password it raises `NotImplementedError` ("compression type ... not supported"), since stdlib `zipfile` never implemented WinZip's AES scheme at all. `pyzipper` (MIT-licensed, wraps `pycryptodomex`) fills that specific gap.

**`archive_extract.py`**: new `PasswordRequired` exception (`wrong_password: bool` distinguishes "never asked" from "asked, was wrong" without parsing message text). `extract_archive()` gained an optional `password` param. `.zip` tries plain `zipfile` first (the common, fast, dependency-free path for the overwhelming majority of real mod zips, unchanged for anyone not hitting encryption) and only falls back to `pyzipper` on the two stdlib failure modes above; `.7z` threads `password` straight into `py7zr.SevenZipFile`'s own constructor and translates its `PasswordRequired`/`CrcError`/`Bad7zFile` into this project's own exception type. `.rar` stays unsupported for passwords (deliberately -- `tar.exe`, the whole reason `UnRAR.exe` was removed per item #43, has no decryption support at all; re-adding it would need re-bundling a real RAR-decrypt binary, undoing that whole effort).

**GUI wiring**: `App.ask_password()` (mirrors the existing `ask_yes_no()`/`show_info()` pattern, routed through the already-established `_run_on_main_thread()` cross-thread mechanism so it's safe to call from any background worker). `App.extract_archive_prompting()` wraps `extract_archive()` in a retry loop, prompting via `ask_password()` on `PasswordRequired` and looping until it succeeds or the user cancels (which re-raises, caught by each call site's existing error handling same as any other extraction failure) -- returns `(mod_root, password)` so a caller that re-extracts the SAME archive later doesn't have to prompt twice. Both retarget dialogs (armor and weapon) needed this: their "Generate" step re-extracts from the ORIGINAL archive path independently of the earlier detection step (a fresh temp dir each time, always has), so the resolved password from detection is stashed in each dialog's own `state` dict and threaded through to `retarget_archive_multi()` (both `slot_retarget.py` and `weapon_retarget.py` gained a `password` param for exactly this). CLI (`auto_fix.py`) gained a plain `--password` flag with a clear stderr message pointing at it on failure.

**A real PyInstaller gotcha, caught before it shipped**: `import pyzipper` lives inside `_extract_zip()`'s function body (lazy, only needed on the rare encrypted-archive path) -- exactly the pattern this project's own README already warns about ("must list every module that's only ever imported lazily inside a function as --hidden-import, or PyInstaller's static analysis misses it"). First build showed zero mention of pyzipper anywhere in the build log and zero pyzipper files under `dist/` despite compiling and launching fine -- would have shipped a silent crash the first time any real user hit a password-protected archive. Fixed with `collect_all('pyzipper')` in `MHWmodfixer.spec` (a real package with its own submodules, not a bare `hiddenimports` string). Its `pycryptodomex` dependency needed no separate collection -- PyInstaller's own bundled `hook-Cryptodome.py` already handles it (confirmed: adding a redundant `collect_all('Cryptodome')` on top pulled in Cryptodome's entire `SelfTest` suite as pure bloat, removed).

Verified end-to-end against the real "Echo" archive: no-password/wrong-password/correct-password all produce the right `PasswordRequired` state (`wrong_password` flag correct in both failure cases), correct-password extraction produces byte-identical content to a direct `pyzipper` reference extraction; a normal unencrypted zip (SilverWolf, the standard regression mod) still extracts via the unchanged fast path, confirming zero behavior change for the common case. i18n audit confirms all 3 new keys (`dlg_archive_password_title`, `ask_archive_password`, `ask_archive_password_wrong`) have full 5-language coverage from the start, per item #39's own standing lesson about not deferring non-Korean text. Rebuilt exe launches cleanly with `_internal/pyzipper` confirmed present.

### 53. "Change Target Armor" warns when a mod ships pieces beyond the standard 5 (2026-08-14)

Real, empirically-confirmed case: a Chinese-community-pipeline mod ("DOA - Ninja 2015- Ayane") bundles a 6th file (`ch03_058_0016.mesh`, mesh-only, no mdf2, no pfb -- almost certainly a wrist accessory) beyond the standard 5 armor pieces (Arm/Body/Helm/Leg/Waist). Retargeted to one slot, it rendered correctly; retargeted to a DIFFERENT slot, the wrist piece vanished/rendered wrong -- same mod, same code, different outcome purely depending on target. Root cause: `armor_slots_ch03.json.gz`'s own compatibility data (`bake_armor_slots.py`) only ever tracks pieces 1-5 -- a "piece 6" is relocated by the same blind part-level renaming as everything else (`retarget_tree()` doesn't special-case piece count), but there's no data anywhere in this project to verify whether the CHOSEN target's real vanilla structure has an equivalent slot for it. No vanilla file was found at the source's own `058/001` piece-6 path either (checked directly against the live game), so this bonus content loads through some mechanism this project hasn't mapped -- consistent with the same "mesh-only piece 6, no mdf2" shape seen earlier the same day on Mangie "Fluffy Bunny" (item #37/#52 context), suggesting this is a real, recurring pattern across mods, not a one-off.

**Fix, scoped to detection/warning only (not a structural fix -- there's no data source yet to actually verify compatibility for these)**: `_open_retarget_dialog()` in `gui.py` gained `extra_pieces_label`, a warning shown per selected slot (not per candidate, since it's a property of the SOURCE mod regardless of target) whenever `ModSlotGroup.pieces_shipped` contains anything beyond `{1,2,3,4,5}`. New i18n key `note_extra_pieces_unverified`, all 5 languages from the start. Verified directly against the real Ayane mod: `detect_mod_slots()` correctly reports `pieces_shipped={1,2,3,4,5,6}`, warning text renders correctly in all 5 languages, dialog opens without exception.

Not fixed here, flagged for a future session if it recurs enough to be worth the effort: actually determining HOW a piece-6-style bonus mesh gets loaded (which vanilla/engine mechanism references it, given no mod-bundled pfb and no vanilla file found at the same path) would be needed before this could become a real per-target verification instead of a blanket warning.

### 54. Weapon retargeting: state-dependent visual effects (sheathed vs drawn glow) not verified by the compatibility check either (2026-08-14)

Real case, same day as item #53's armor "extra piece" finding, same root shape but for weapons: user retargeted a real Great Sword Artian-appearance mod ("GS_Artian_LVL5", targeting `it00/00/0002`) to `it00/00/0007` after first repairing it (`process_mod`, textures_restored=21, no pfb touched -- mod ships none). In-game, the weapon's glow VFX (normally sheathed=off, drawn=on, via a `Group_[state-code]_Sub_...`-style submesh convention -- see item #19's research notes) rendered **always on**, both sheathed and drawn.

**Ruled out by direct byte-level verification, not assumption**: the mod's own `lambert2` material carries the real "glow on" prop signature (`UseWaveEmit=1`, `RayTrace_Emissive_Booster=300`, `Emissive_Intensity=8`) while its other 3 materials (`lambert11`/`lambert12`/`lambert13`) sit at the "off" baseline (`UseWaveEmit=0`, `RayTrace_Emissive_Booster=1`) -- confirmed unchanged, byte-for-byte, at every stage: raw original, after repair, and in the final retargeted+repaired file the user actually tested in-game. The mesh itself (which is what actually encodes which submesh belongs to which state) is confirmed **byte-identical** between the original `0002` slot and the final retargeted `0007` slot -- repair never touches mesh content, and weapon retargeting (`retarget_tree()` in `weapon_retarget.py`) is confirmed pure filename/path renaming, same principle already verified for armor.

**Leading hypothesis, not yet confirmed**: since this mod ships no pfb of its own, sheathed-vs-drawn submesh visibility switching is driven by the TARGET slot's own vanilla pfb (unmodified, since nothing overrides it) -- if `it00/00/0007`'s real vanilla pfb doesn't gate submesh visibility by state the same way `it00/00/0002`'s does (or doesn't gate it at all), everything would render unconditionally regardless of how correct the mod's own mesh/material content is. This would be the exact same shape of gap as item #53 (armor's extra-piece warning) -- `find_compatible_weapon_targets()`'s physics-component check (ChainSetting/Chain2/etc.) has no way to verify state-dependent visual behavior at all.

**Update, same day: the pfb-level diff was done, and it points at a real, specific, structural difference.** Fetched both slots' real vanilla pfbs directly from the live game (`natives/stm/GameDesign/equip/_prefab/weapon/wp00/00/000{2,7}/it0000_000{2,7}_0.pfb.18`) and diffed both their RSZ instance-type multiset (via `pfb_fix._parse_rsz()` + the registry) and their full resource-string list (via `pfb_fix._scan_utf16_strings()`). The instance-type multisets are nearly identical (57 vs 56, differing only by one extra `via.motion.JointConstraintsLayer` in `0002` -- not obviously the mechanism). The resource strings reveal the real difference:

- `0002`: `Motion/Player/WeaponGimmick/Wp00_01/Wp00_01Gimmick.motbank` -- a **weapon-specific "01" gimmick bank**.
- `0007`: `Motion/Player/WeaponGimmick/Wp00/Wp00Gimmick.motbank` -- the **plain, generic per-type gimmick bank**.

"WeaponGimmick" is this engine's own term for special per-weapon animation-linked behavior (motion-bank-driven, not a static mdf2/mesh property) -- exactly the kind of mechanism that would drive an animation-event-triggered sheathed/drawn material swap. `0002` is wired to a dedicated gimmick bank; `0007` isn't, it just uses the type-wide default. This strongly explains the symptom: with no gimmick-driven toggle at the target slot, whatever the mesh/material's own baked-in state is (here, the "on" glow values) just renders unconditionally, since nothing ever switches it off.

Also visible in the same diff, likely related rather than incidental: `0002` references TWO jcns files (`..._DP.jcns` and `..._MB.jcns`, suggesting two distinct joint-constraint configs) and has no chain2/chainwnd reference at all, while `0007` has exactly one plain `.jcns` plus a real `.chain2`/`Chain2Wind.chainwnd` reference -- consistent with `0002` being a slot custom-built for this specific weapon's special presentation (dual jcns states, dedicated gimmick bank) rather than an ordinary physics-bearing weapon slot like `0007`.

**Still not fixed, and likely not closeable the way item #51 was.** Unlike armor's "piece 6" (a simple `pieces_shipped > 5` count this project already tracks), there's no existing per-slot data in `weapon_slots.json.gz` that records whether a slot has a dedicated gimmick bank -- adding that would mean baking a NEW field (scanning every slot's real vanilla pfb for its gimmick-bank string, comparing against the type's own default) into `bake_weapon_slots.py`, not just reading data already collected. Worth doing in a future session if this recurs, but not attempted here -- flagging the exact resource-string pattern (`WeaponGimmick/<Wp NN>_<variant>/...` vs the bare `WeaponGimmick/<Wp NN>/...` default) is enough for whoever picks this up next to go straight to baking the check rather than re-deriving it.

### 55. Item #54's weapon-gimmick investigation resolved -- confirmed via live A/B testing, not just theory (2026-08-14)

Direct continuation of item #54 (GS_Artian_LVL5/"Varianza", retargeted `it00/00/0002` -> `it00/00/0007`, sheathed/drawn glow stuck permanently on). User pushed to keep investigating with live REFramework data rather than accept the earlier static-analysis conclusion.

**Live GameObject inspection, via a proper working REFramework Lua access chain** (worth recording for reuse -- getting here took several wrong turns): `sdk.get_managed_singleton("app.PlayerManager")` -> `:call("getMasterPlayer")` -> `:call("get_Character")` (type `app.HunterCharacter`, NOT `get_Chara` -- that method doesn't exist on `app.cPlayerManageInfo`, the actual type `get_MasterPlayer()` returns) -> `:call("get_Weapon")` (type `app.Weapon`) -> `:call("get_GameObject")`. `HunterCharacter:get_IsWeaponOn()` gives the real live sheathed/drawn boolean. A Transform's `get_ChildCount()`/`get_Child(i)` DON'T exist on this engine's Transform type -- only `get_Children()` (a collection, same `:get_Count()`/`:get_Item(i)` shape as `GameObject:get_Components()`) and `get_Child(i)`.

**First real finding**: `ace.model_parts_manager.ModelPartsManager2` (a component on the weapon's own root GameObject) has `setPartsDisp(index, bool)`/`getPartsDisp(index)`. A live sheathed-vs-drawn A/B diff (captured via the exact confirmed-state each time, not guessed) found exactly 2 part indices that flip in sync with `IsWeaponOn`: part 1 (off when sheathed, on when drawn) and part 2 (the inverse) -- every other index (0, 3-15) identical in both states. A companion script (`mhwmodfixer_gs_artian_lvl5_parts_fix.lua`, written to `reframework/autorun`, NOT part of this repo) was built to call `setPartsDisp(1, isOn)` / `setPartsDisp(2, not isOn)` on every `IsWeaponOn` change, gated to only affect a GameObject whose own name contains "0007" (the retargeted slot) so no other weapon is ever touched. Confirmed via debug JSON logging (both a per-frame snapshot and an 8-entry rolling change-history file) that the calls fire correctly and read back as applied, in perfect sync with real player draw/sheathe actions.

**Yet the visual glow stayed on regardless.** `via.render.Mesh` itself turned out to have its OWN, separate `getPartsEnable(index)`/`setPartsEnable(index, bool)` API (found by dumping its full method list, since the earlier keyword-filtered search only turned up unrelated `BeautyMask`/`LodGroupHash` methods) -- confirmed to read back the identical live values as `ModelPartsManager2`'s parts 1/2 (same underlying state, two API surfaces onto it), ruling out "wrong API" as the explanation.

**The decisive test**: added a `FORCE_STATE` override to the companion script -- when set (not `nil`), it forces parts 1/2 to a fixed state EVERY FRAME regardless of real draw state, letting the user visually A/B the two states directly rather than relying on the automatic (and possibly still-buggy) toggle. `FORCE_STATE = true` (part 1 on) showed the transformed/glowing shape, as expected. `FORCE_STATE = false` (part 2 on, part 1 off) showed **the identical transformed/glowing shape -- no visual difference at all**, confirmed directly by the user.

**Conclusion, now confirmed rather than hypothesized**: parts 1/2 are a real, correctly-identified, correctly-controllable toggle at the engine level -- but this specific fan-made mod's own custom mesh does not contain a genuinely different "sheathed" shape in part 2's slot at all (empty, or duplicate of part 1's geometry). The user's own suspicion from earlier in this same investigation ("이 파일이 예전 버전이라 아직 그 발광 변신 기능이 적용되지 않은 버전 아니니?") was correct: the mod itself never implemented the transform, independent of which slot it's equipped at, independent of any file/script fix. This matches this session's own earlier research note (item #19's external-resources map, Caimogu's "从零开始做外观mod教程"): "the material part is still being researched" -- full shader/material-driven effects are an acknowledged, community-wide unfinished area for MHWilds modding tooling as of this session, and this specific case (a multi-state transforming weapon) is a clear instance of that gap. Not fixable by MHWmodfixer, a companion script, or any other means available to this project -- the underlying geometry data simply isn't there to show.

`mhwmodfixer_gs_artian_lvl5_parts_fix.lua` and its `FORCE_STATE` override are left in place (set back to `nil`, i.e. inert/no-op by default) in case a future session wants to revisit a DIFFERENT weapon mod that turns out to have this exact same "extra piece"-shaped gap (mirroring item #53's armor finding) but whose mesh genuinely does contain both states -- the working live-inspection technique and the two-tier ModelPartsManager2/via.render.Mesh API are now a proven, reusable path to that answer without needing another multi-hour live debugging session from scratch.

### 56. Merge reconciliation: two sessions independently re-investigated subid=01/03 and reached different conclusions on subid=01 -- #47's finding wins, with a stronger cross-check (2026-08-14)

Two sessions branched from the same point (right after #45) without either seeing the other's work, since neither had pushed yet when the other started. Both independently noticed `it10/03/*` wasn't a weapon list (Kinsects) and named all 21 entries -- **#46's `resolve_rodinsect_names()` (official `RodInsect.msg.23` game text, cross-matched via exact zh_cn text equality) and #50's `CONFIRMED_MANUAL_NAMES` block (Kiranico's kinsect database, cross-matched via 5 independent semantic anchors) agree on all 21 names, on every language present in both (en/ko/ja/zh_cn)** -- two fully independent methods and sources converging exactly is about as strong a confirmation as this project ever gets. `resolve_rodinsect_names()` is kept as the active mechanism (it also resolves zh_tw, which Kiranico doesn't have for this game) -- #50's now-redundant `CONFIRMED_MANUAL_NAMES.update({...})` Kinsect block was removed from `bake_weapon_names.py` as dead weight (`main()`'s merge order always ran `resolve_rodinsect_names()` last anyway, so it was silently overwriting #50's own values with identical ones -- functionally harmless, but confusing to leave two mechanisms doing the same job). No data changed as a result of this dedup; both sources already agreed.

**subid=01 (32 real weapon-file entries across 13 types) is where the two sessions genuinely disagree, and it needed a real tiebreak, not just a merge.** #50's session concluded these are legitimate pre-"Hope tier" CUT CONTENT (real Iron/Bone-branded weapons predating a Wilds-specific rework, with no official Wilds localization to find) and kept the community CSV's zh_cn labels attached to `it*/01/*` -- confirmed with that session's own user. #47's session concluded the community CSV's own "01_<iid>" rows don't actually describe subid=01 at all -- they're mislabeled subid=10 data under a different internal divisor -- and deleted them.

**These aren't actually contradictory theories about the SAME question -- they're answers to two DIFFERENT questions, and only one of them is about what the CSV's labels really say:**
- #50's question: "is subid=01 (the real, existing mesh/mdf2 files at `it*/01/*`) legitimate cut content?" -- Yes, very likely, on real evidence (naming convention matches the 20-year MH series' Iron/Bone pattern, and Wilds demonstrably replaced that whole tier with "Hope"-branded content). **This conclusion is NOT contradicted by anything #47 found and still stands.**
- #47's question: "does the community CSV's zh_cn text, when attached to `it*/01/<iid>`, actually describe THAT file?" -- No: checked all 31 non-empty rows by real official-data cross-check (each CSV row's claimed name vs. that exact iid's REAL name at `it*/10/<iid>`, sourced from `WeaponData.cData` via the trusted `_Name`-GUID-to-msg-UUID link, not a guess) and got **31/31 (100%) exact matches to the subid=10 slot, not the subid=01 slot**. The CSV's "01" bucket is real data, just about a different, already-otherwise-named slot.

Put together: subid=01 likely IS real cut content (per #50's own solid reasoning), but the specific zh_cn TEXT previously shown for it was never actually about it -- it was correctly-sourced information about a completely different (already-named) weapon, misattached by a numbering-scheme mismatch between this project and the third-party CSV's own internal conventions. Showing a real name for the WRONG weapon is strictly worse than showing no name, so **#47's deletion is the one that stays** -- subid=01 remains genuinely unnamed, and #50's cut-content theory for what it structurally IS remains the best available explanation, just without any (currently) real name text to attach to it. If a future session wants to try again, the lesson from both investigations combined is specific: a real per-item text match (like #46/#50's Kinsect cross-check) is trustworthy regardless of what a third-party source's internal id scheme means; trusting an id-BUCKET label match (`_COMMUNITY_TRUSTED_SIDS`'s original "01" inclusion) is not, even when a sibling bucket ("00") checked out fine -- different buckets can use different, silently incompatible numbering entirely.

Both sessions' `tools/weapon_slots.json.gz` changes were reconciled by hand (git can't diff a gzip binary): started from #50's version (has the Kinsect names, the sid=10 investigation stats unchanged), added zh_tw to the 21 Kinsect entries per `resolve_rodinsect_names()`'s output, and re-applied #47's deletion of the 31 wrong `it*/01/*` names. Verified after merge: 674 total entries (unchanged), Kinsects have all 5 languages, subid=01 entries carry no `names` field, `weapon_retarget.py` imports cleanly.

### 57. CORRECTION to item #55: the "mesh has no second shape" conclusion was wrong -- isolated to retargeting itself, via a controlled native-slot test (2026-08-14, different session)

A separate session (branched before #56's merge, working the same GS_Artian_LVL5 bug from item #54/#55) pushed the user to test something #55 never isolated: whether the material REPAIR itself (not just retargeting) was contributing to the stuck-on glow. The full raw-vs-repaired material diff (all 4 materials, every property/texture/mmtr/gpbf/shading/alpha field, not just the 9 originally checked) found the repair adds 13 properties the stale mod's material never had at all (`AnimEmit_Blend`, `Basecolor_Mask_Max/Min`, `Fix_BaseColor_to_Emissive`, `OPT_MODE_Detail_Metal_A/B/G/R`, `OPT_MODE_Detail_Roughness_A/B/G/R`, `Use_Basecolor_Mask`), all taken straight from the donor's own current default values -- `AnimEmit_Blend` (donor default `0.0`) was the leading suspect, since "animation-driven emissive blend" is exactly the kind of parameter a WeaponGimmick-bank-triggered transform would need.

**Controlled test, isolating retargeting from repair for the first time**: two zips built with the IDENTICAL repaired material (all 13 new properties present, untouched donor defaults) -- one retargeted to `it00/00/0007`, one left at the mod's own native `it00/00/0002`. **Native slot, repair only, no retargeting: the transform toggles correctly** (confirmed directly by the user, "이건 정상 작동 제대로 하는데?"). This proves the repaired material -- `AnimEmit_Blend=0.0` and all 12 other new properties included -- was never the cause of anything; #55's `FORCE_STATE` A/B test that concluded "the mesh doesn't contain two real shapes" was run EXCLUSIVELY against the weapon at the RETARGETED slot (the companion script's own `TARGET_NAME_SUBSTRING = "0007"` gate ensured this) and never once tested the native slot -- the mesh is fully correct and does contain two real states.

A separate, riskier experiment (forcing `AnimEmit_Blend` to `1.0` instead of the donor's `0.0`, same native slot) caused the game to hang/black-screen at boot -- confirms the property isn't an inert no-op (an unexpected value has a real engine-level consequence, plausibly because `1.0` signals "read from a bound animation-curve resource" this weapon's pfb never wires up) but is NOT the mechanism behind the original bug, since the donor's own default already works fine. This value should not be experimented with further, matching item #18's own established lesson about not pushing forward with more guesses once a wrong value has already demonstrated an undiagnosable boot-time failure -- `0.0` (whatever the donor's own current material carries) is the only confirmed-safe value.

**Real, now-doubly-confirmed root cause**: retargeting alone breaks it, independent of the material fix -- pointing squarely back at item #54's own original finding (slot `0002`'s real vanilla pfb references a dedicated `Motion/Player/WeaponGimmick/Wp00_01/Wp00_01Gimmick.motbank`; slot `0007`'s uses the generic, type-wide `Wp00/Wp00Gimmick.motbank`). This mod ships no pfb of its own, so retargeting it makes it inherit the target's vanilla pfb -- with no dedicated gimmick wiring to trigger the transform at all, the mesh/material sit at whatever static state they initialize to regardless of real draw/sheathe state.

**Practical implication**: the sheathed/drawn transform glow can only work correctly at the mod's own NATIVE slot. Retargeting to a different slot cannot currently preserve this effect -- attaching the source slot's dedicated gimmick-bank reference to a different target's pfb was already attempted earlier in this investigation (a same-length UTF-16LE string substitution failed outright since the two bank path strings are different lengths; a full custom-pfb rebuild with the id substituted made the weapon fully invisible in-game, cause never diagnosed, abandoned). This is a real, confirmed capability gap, not a guess. `mhwmodfixer_gs_artian_lvl5_parts_fix.lua` (the ModelPartsManager2 companion script from #54/#55) is now understood to be irrelevant to this bug -- parts 1/2 are a real toggle, but not the mechanism behind this weapon's transform (which is pfb/gimmick-bank-animation-driven, at a level the Lua companion-script approach never needed to intervene in).

### 58. Weapon retargeting now warns when a source slot has a dedicated WeaponGimmick bank the target doesn't share (2026-08-14, same session as #57)

Turned #57's finding into a real, shipped warning, the same "detect and warn, don't silently drop" pattern #53 already established for armor's extra-piece gap.

**`tools/bake_weapon_slots.py`**: `scan_weapon()` now extracts every `WeaponGimmick`-containing resource string from a slot's own vanilla equip pfb (`pfb_fix._resource_strings()`, the pre-RSZ header region -- same helper already used for physics-type scanning) and stores it as `gimmick_bank` (a string, or a list when the header also carries a companion `_mcb.user` reference alongside the `.motbank` itself -- confirmed real on both `it00/00/0002`/`0007`). After baking every slot, a post-pass computes each weapon TYPE's majority ("generic default") gimmick bank by plain vote count and flags any slot whose own bank differs from that majority as `special_gimmick: true` -- 102 of 622 slots project-wide, confirmed correct on the known pair (`it00/00/0002` = flagged/dedicated `Wp00_01Gimmick`, `it00/00/0007` = not flagged/generic `Wp00Gimmick`). Computed by majority vote rather than a hardcoded per-type pattern string, so it stays correct across a future title update without needing a code change.

**`weapon_retarget.py`**: `TargetWeaponCandidate` gained `loses_gimmick: bool`. `find_compatible_weapon_targets()` sets it whenever the SOURCE slot is `special_gimmick` and a given target candidate's own `gimmick_bank` doesn't match the source's exactly -- true for essentially every other slot in practice (a dedicated gimmick bank is normally unique to its own weapon), which is the correct, honest signal: retargeting AWAY from a special-gimmick slot to basically anywhere else loses that slot's animation-driven behavior (transform glow, etc.), regardless of which target is picked. This is INFORMATIONAL only, never blocking -- most mods sitting at a special-gimmick slot are ordinary reskins that never used the effect at all.

**GUI**: new i18n key `note_weapon_loses_gimmick` (all 5 languages from the start), appended to the existing `_weapon_note_for()` helper in `gui.py` alongside the physics-mismatch note (both notes concatenate when both apply).

**Merge note**: this item's own `tools/weapon_slots.json.gz` change (adding `gimmick_bank`/`special_gimmick` to every entry) had to be reconciled against item #56's own independent `weapon_slots.json.gz` reconciliation (different session, same day) -- resolved by re-running `bake_weapon_slots.py` (this item's own new gimmick-bank scan) THEN `bake_weapon_names.py` (which already carries #56's merged name-correctness fixes, since that file's own text changes merged cleanly with no conflict) on top of the git-merged codebase, rather than hand-editing the binary gzip. Verified after merge: `it00/00/0002`/`0007` still resolve to their real names AND carry the correct `special_gimmick`/`gimmick_bank` data; `find_compatible_weapon_targets()` on a synthetic `it00/00/0002` source correctly flags `loses_gimmick=True` on `it00/00/0007`. Headless GUI smoke test (the established `TkinterDnD.Tk()` + real mainloop pattern) confirms the weapon retarget dialog still opens without exception. No other module touched -- the core repair pipeline (`auto_fix.py`/`slot_merge.py`/`pfb_fix.py`/armor's `slot_retarget.py`) is unaffected.

### 59. Weapon retargeting can now pull ONE appearance out of a multi-page mod, repair it, and hand it back as a standalone mod (2026-08-15)

Real case: a user had "Summer Fleet Weapons" (Nexus, `joolsdgg`), a FOMOD-style archive bundling several unrelated weapon-type reskins as separate top-level pages (`anchor hammer` = Hammer, `fleet anchor horn` = a pak-bundled page, three `fleet harpoon*` pages = Insect Glaive variants, plus a `[00]cover` splash page), broken by the current title update. They only wanted the Hammer skin. Handled by hand first (extract the `anchor hammer` folder, run it through the normal repair pipeline, strip its `AddOnFor=Summer Fleet Weapons` line -- which named a base bundle that wouldn't be present in a standalone extract and confused Fluffy's installer -- and set `NameAsBundle`) -- then, per the user's own follow-up ("이런 기능을 넣는게 가능한가"), turned into a real feature in the "적용 무기 변경" dialog.

**New `mod_pages.py`**: `detect_mod_pages(mod_root)` finds every top-level folder carrying its own `modinfo.ini` (reusing `fluffy_repackage.py`'s own private `_modinfo_folders()`/`_read_modinfo()`/`_write_modinfo()` helpers rather than duplicating that logic -- both modules are about the same "FOMOD page" concept from opposite ends: `fluffy_repackage.py` WRAPS loose extras into new pages, this module PULLS ONE EXISTING page back out). `extract_page_standalone(page, out_root)` copies just that page's own files, flattened to `out_root`'s top level, and rewrites its `modinfo.ini`: drops `AddOnFor` (case-insensitively) and ensures `NameAsBundle` is set (falls back to the page's own `name`, then its folder name) -- the exact fix applied by hand to the real mod, now automatic.

**GUI wiring** (`_open_weapon_retarget_dialog()`): after extraction, `detect_mod_pages()` runs before weapon detection. A page with no real content (screenshot+blurb only, e.g. `[00]cover`) is filtered out by checking for an actual `.pak` file or a `natives/`-containing path among its files -- deliberately NOT by trusting a Fluffy-specific `DummyMod` flag some authors may not set. Zero or one real page: unchanged behavior, straight to weapon detection. More than one: a new modal page-picker Toplevel (name + description columns, matching this dialog's own existing Treeview conventions) blocks via `wait_window()` until the user picks one or cancels. Once picked, `_extract_and_repair_page()` runs the chosen page through the SAME repair pipeline the main window's own repair flow uses -- `shutil.copytree` then `process_mod()` then `repackage_for_fluffy()`, mirroring `auto_fix.py` CLI's own `main()` sequence exactly -- into a persistent staged directory, THEN runs weapon detection on the repaired result.

**A real bug found and fixed while wiring this in**: the dialog's existing `do_pick()` worker deleted its own extraction tempdir (`work`) in a `finally` block immediately after extraction, before the caller ever got to read `mod_root` from it -- harmless in the ORIGINAL code (where `detect_mod_weapons()` ran INSIDE that same try block, before cleanup), but a real bug once page detection was split out to run first and weapon detection got deferred to a later callback. Fixed by not cleaning up `work` in the worker at all -- it's now tracked in `state["_pick_work_dir"]` and torn down (alongside any page-repair staging dir) by a new `_clear_temp_dirs()` helper, called both at the start of a fresh `do_pick()` and on dialog close. Caught by a real headless GUI test that hung indefinitely rather than by inspection -- worth remembering alongside item #44's own "not cleaned up here" lesson about background-worker lifecycle bugs being easy to introduce silently when refactoring an async flow into more steps.

**A second real bug, in the TEST harness itself, not the product**: an early version of the end-to-end test used manual `root.update()` polling in a loop instead of a real `root.mainloop()`, and the picker dialog appeared to "never open" -- actually a re-confirmation of item #35's own already-documented finding ("manual `update()` pumping alone raises `RuntimeError: main thread is not in main loop`" for cross-thread `.after()` callbacks) that this session re-discovered the hard way before remembering to check for it. Once rewritten around `root.after()`-scheduled steps + a real `mainloop()`, the picker, page selection, and weapon detection all worked correctly.

**`do_generate()`** now uses `state["effective_mod_root"]` (the repaired, page-narrowed staging directory) instead of re-extracting the ORIGINAL, still-multi-page archive from `file_var.get()`, when a page was selected -- `retarget_archive_multi()` already accepts a plain directory just as readily as an archive path and skips its own extraction step entirely when given one, so no change was needed there. The output filename default also prefers the selected page's own display name (`state["_page_name"]`) over the whole bundle's filename stem, so a "Summer Fleet Weapons..." archive correctly suggests "Fleet Anchor (retargeted).zip", not the original bundle's name.

**Verified end-to-end via a real headless GUI test** (`TkinterDnD.Tk()` + `root.mainloop()`, driving the actual dialog code -- not a reimplementation): picking the real "Summer Fleet Weapons" archive correctly shows 5 selectable pages (the cover page filtered out), picking "Fleet Anchor" correctly extracts+repairs+detects exactly `it04/01/0000` (2 files), and clicking through Leave-Unchanged + Generate produces a valid output zip. **The output's `mdf2`/`mesh` bytes are byte-for-byte identical to the hand-built delivery** from earlier in this session -- the strongest available proof the automated path reproduces the manual fix exactly, not just "looks similar." A separate regression test confirms a normal single-weapon mod with no FOMOD page structure at all (GS_Artian_LVL5) is completely unaffected -- no page picker appears, detection proceeds exactly as before. i18n audit: 153/153 keys with full 5-language coverage after the 6 new keys this feature added.

**Follow-up, same day: `mod_pages` was missing from `MHWmodfixer.spec`'s `hiddenimports`.** It's only ever imported locally inside `_open_weapon_retarget_dialog()`, the exact class of gap this project's own README/CLAUDE.md already warns about repeatedly (a lazily-imported, function-body-only import PyInstaller's static analysis never sees) -- confirmed missing from the first rebuilt `dist/` by directly checking the PYZ archive's own table of contents (`PyInstaller.archive.readers.ZlibArchiveReader`), not just by inspecting loose files on disk (pure-Python modules live inside the PYZ, not as loose `.py`/`.pyc` files, so a plain filesystem `find` for the module name gives a false negative either way). Added `'mod_pages'` to `hiddenimports`, rebuilt, re-verified present in the PYZ, smoke-tested launch.

### 60. Weapon names now show each model's FINAL tier, not its first -- multi-language by construction (2026-08-15)

Real user complaint: retargeting shows "Jawblade I" for `it00/00/0000`, but that model's actual highest tier (still the same 3D model) is a completely different name, "Giant Jawblade" -- not just a numeral bump. `resolve_names()`'s own representative-row selection (`best_row_by_model`, dedup by `_ModelId` since several tiers commonly share one visual) picked the LOWEST msg index per model -- i.e. always the first, least-upgraded tier, which is the name a player is least likely to actually recognize, since playing a weapon to its final tier is the normal way it gets used.

**Fix**: flipped the comparison (`idx > best_row_by_model[model_id][0]` instead of `idx <`) in both `resolve_names()` (the active static-file-reading path) and `resolve_names_from_live_dump()` (the dormant live-dump alternative, fixed for consistency even though nothing currently invokes it). Confirmed real and exactly as expected on 3 live examples before shipping: `it00/00/0000` "Jawblade I" -> "Giant Jawblade" (name changes entirely, not just a numeral), `it00/00/0001` "Dosha Fellslayer I" -> "III" (plain tier bump, final), `it00/00/0006` "Quematrice Espada I" -> "IV" (same).

**Multi-language is automatic, not a separate concern** -- each msg entry already carries all 5 languages (ko/en/ja/zh_tw/zh_cn) together in one row; switching which ROW gets picked as representative changes all 5 languages' displayed text at once, in lockstep, by construction. Verified directly: `it00/00/0000`'s new names are `{'ko': '그레이트아기토', 'en': 'Giant Jawblade', 'ja': 'オオアギト', 'zh_tw': '大獸顎', 'zh_cn': '巨颚'}` -- a real, different, fully-translated name in every language, not an English-only fix.

**`CONFIRMED_MANUAL_NAMES` overrides (Artian tiers, applied in a later unconditional merge pass) are completely unaffected** -- `it00/00/0002` still resolves to "Varianza"/"발리안차" exactly as before, confirmed directly. Also spot-checked that a genuinely FINAL-tier subid=100 (Artian) unique name (`it04/100/0002` = "Moteurvankel"/"모토반켈") comes out correctly from the ordinary highest-index pick too, independent of any manual override -- consistent with item #47's earlier finding that a model_id's true final tier is often a wholly unique name, not a numbered one.

**Re-running the bake a second time in the same session produced a confusing-looking but harmless count shift** (`subid=100 entries added name-only` dropped from 52 to 0, `matched` jumped by the same ~66) -- NOT a regression: the first run already wrote those 52 name-only subid=100 entries into `weapon_slots.json.gz` on disk, so the second run's own `resolve_names()` output now finds them already present in the table and counts them as "matched" instead of "newly added". Verified the actual data is identical either way (total entry count stayed at 674 across both runs) -- worth remembering this specific counter is not idempotent-run-safe to read literally, only the underlying `weapon_slots.json.gz` content matters.

Rebuilt and redeployed the exe with the refreshed `weapon_slots.json.gz` (same process as item #59's own rebuild -- `mod_pages` hiddenimport fix already included).

### 61. Pak-bundled wholesale-replace was silently destroying custom VFX pfbs the registry can't verify at all -- real regression traced and fixed (2026-08-15)

Real report: "Wyvern Impact2.2" (a Great Sword reskin at `it00/00/0002` -- the "Varianza" slot, same one investigated in items #52/#54/#57/#58 the same day, unrelated coincidence: it just happens to be a popular custom-slot target) has a custom rocket-burst VFX at Great Sword's level-3 charge. The user had a REAL, confirmed-working reference build from an earlier MHWmodfixer run alongside a freshly re-run repair, and the fresh one lost the effect -- but a naive hash comparison against the wrong reference file initially suggested no regression at all (see below, worth reading as a cautionary tale about verifying you're diffing the RIGHT two files before trusting a "byte-identical, no bug" conclusion).

**First diagnostic pass went down a wrong path.** Compared a "known good" reference the user had supplied earlier in an unrelated context against a fresh repair and found them byte-identical -- concluded (wrongly) there was no regression. The user then supplied THREE clearly-relabeled files (`(retargeted)`, `(workling)` = the REAL confirmed-working old build, and plain `_fixed.zip` = today's broken build) after being asked to "give it again so you don't get confused" -- the earlier "known good" reference turned out to actually be byte-identical to the BROKEN build, not the working one. Lesson: when a user says "compare A vs B," get an unambiguous, explicitly-labeled pair before drawing conclusions -- an implicitly-assumed reference file can be wrong even when the investigation methodology (byte-diffing) is completely sound.

**With the real pair in hand, found the actual difference immediately**: 2 of the pak's 11 entries differ between working and broken. RSZ-parsing them (`pfb_fix._parse_rsz` + the type registry) showed both are `via.effect.script.EPVStandardData`/`app.EPVExpertCommonData`/`app.EPVExpertHitData`-family instances -- unambiguously a VFX/particle-effect-parameter pfb, not the weapon's standard equip pfb. The WORKING build's copy of these entries is BYTE-IDENTICAL to the mod's own RAW original (confirmed directly) -- the old repair left them completely untouched. The BROKEN build's copy is a wholesale donor replace (matches the CURRENT live game's own donor bytes at that hash exactly, confirmed via `game.read_by_hash()`).

**Root cause, confirmed by direct testing**: `rsz_layout.fits_current_layout()` returns `False` for BOTH the mod's own bytes AND the CURRENT DONOR's own bytes for this instance -- i.e. even a guaranteed-current, freshly-read-from-the-live-game donor fails to structurally verify. Since a genuinely current file can only fail this check if the REGISTRY itself has no reliable field data for one of its types (matching item #21's own established finding that native/effect-script types are inherently under-covered in every available community RSZ dump), this isn't evidence of staleness at all -- it's a registry coverage gap. `_plan_pak_rsz_entry()` (pak_mod_fix.py) already refuses a CRC-only patch in exactly this situation (`_crc_only_fix(..., require_fits=True)` already checks the MOD's own fits status and correctly declines) -- **but the fallback path after that refusal, `_find_substitution()` + wholesale donor-replace, had NO comparable check at all**, and happily substituted in the donor's bytes based purely on a resource-string-closeness heuristic, with zero structural verification on either side. That's how a real custom VFX pfb this project has no way to verify gets silently discarded as if it were routine staleness.

**Fix**: `_plan_pak_rsz_entry()` now checks `rsz_layout.fits_current_layout(donor_info)` right after the CRC-only tier refuses -- if the DONOR itself doesn't verify as fitting (proving OR merely failing to disprove a registry gap, both treated the same conservative way `_crc_only_fix()` already treats mod-side unverifiability), the entry is marked `unresolved` instead of falling through to substitution, unless `force_unresolved_pfbs` is explicitly enabled (matching this project's standing "ambiguous cases stay opt-in" convention everywhere else). Confirmed real: BOTH the mod's own bytes and the live donor fail identically here, so this isn't a borderline call -- it's the clearest possible "we have zero verified ground truth for this type family" signal available.

**Verified three ways**: (1) re-running repair on the raw mod now produces `(0 pak-bundled pfb/user/scn file(s) fixed, 2 left unresolved)` instead of silently replacing them; (2) the new output's VFX pfb entries are now **byte-for-byte identical to the confirmed-working `(workling)` reference build**, both entries, full pak; (3) the standard regression suite (SilverWolf: `fixed=1 texture_paths_restored=24`, DoA Raise the Sail: `fixed=4 already_current=1 texture_paths_restored=208`) reproduces the documented baseline exactly -- this fix only changes behavior in the specific "donor also fails to fit" case, which neither regression mod's own resolved pfb/user entries hit.

**Scope note**: this doesn't explain how the OLD "working" build avoided this bug in the first place -- most likely it predates `require_fits` existing at all for this code path (item #9), and simply hit a lucky zero-CRC-change result under the old, less-strict logic. Not investigated further since it doesn't matter for the fix going forward; what matters is the CURRENT code now handles this case safely.

Delivered as `Wyvern Impact2.2 (fixed, VFX preserved).zip` in `C:\Users\User\Desktop\CC download\`.

### 62. "GS_Artian_base" false alarm -- the RAW mod never had the transform effect at all, not a repair regression (2026-08-15, same day)

A separate weapon report, same day as items #57/#61, initially looked like a third instance of the same "repair kills a transform effect" pattern: "GS_Artian_base" (`it00/10/0003`, no bundled pfb) reportedly lost its charge-transform VFX after repair. Ran the exact same exhaustive material diff used for items #57/#61 (every prop, texture, mmtr, shading_type, alpha_flags, gpbf, for all 4 materials) and found **zero functional differences** between raw and repaired -- every prop value the mod itself set is preserved exactly, every texture path matches, and the one newly-added property (`AnimEmit_Blend`) was independently cross-checked against 26 real, currently-shipped vanilla Great Sword materials using the identical shader (including `it00/00/0002`/Varianza itself) -- **all 26 use `AnimEmit_Blend=0.0` with no exception**, confirming (again) it's a universal correct default, not a per-weapon tunable that could plausibly need a different value.

With the material file provably clean, the only remaining way to isolate the report was the same discipline #52/#57 already established: **test the RAW, unfixed mod directly** rather than reasoning further from file diffs alone. The user did, and confirmed the transform effect is **absent in the raw mod too** -- this mod never had the effect in the first place; nothing about MHWmodfixer's repair removed anything. Worth recording specifically because the report's own framing ("this used to work when repaired before, same as that other mod") initially suggested a real regression worth chasing with the same rigor as #61 -- but the file-level investigation, once exhaustive, pointed the same direction #52 already established for a different mod: verify the RAW baseline before trusting a "the fix broke it" narrative, even when everything else about the report sounds plausible and the investigator (this session) already has two confirmed real regressions fresh in mind from the same day. No code change; no delivery beyond the diagnostic repair build already sent for comparison.

### 63. RSZ dumper (Dump SDK + Scylla) attempt abandoned -- MHWilds' Denuvo blocks x64dbg attach; a routine registry refresh confirms no upstream progress either (2026-08-15/16)

Prompted by item #61's own scope note (this project has no way to migrate a genuinely-reshaped native/effect-script type, only refuse safely), the user wanted to try building this project's own authoritative RSZ registry from scratch via REFramework's own documented pipeline (`reversing/rsz/` in the REFramework source -- see the "External resources map" research from an earlier session, already in this file). Real progress made on the TOOLING side before hitting a hard wall:

- Installed Python 3.9 (via winget) in a dedicated venv at `D:\RSZDumperPipeline\venv39` (the dumper scripts require <=3.9, undocumented reason per their own readme) with `fire`/`numpy`/`unicorn==2.1.4`/`capstone`/`pefile` installed.
- Installed x64dbg (via winget) -- confirmed it ships `Scylla.dll` bundled already, no separate download needed.
- **Real gotcha, not obvious from the docs**: x64dbg only loads plugins with a `.dp64` (64-bit) / `.dp32` (32-bit) extension from its own `plugins/` subfolder -- a plain `.dll`, even the exact right file sitting in the plugins folder, is silently ignored with no error shown, which is why "Plugins -> Scylla" showed nothing at first. Renaming `Scylla.dll` -> `Scylla.dp64` and restarting x64dbg fixed detection (confirmed via a real web search of x64dbg's own plugin docs, not guessed).
- **Hard wall**: attaching x64dbg to the running `MonsterHunterWilds.exe` process hangs/crashes the game immediately -- confirmed via web search that MHWilds ships with **Denuvo Anti-Tamper DRM**, still active as of this session (no confirmed removal patch found, unlike Monster Hunter Rise which had Denuvo removed and replaced with Enigma/Obsidian back in January 2024). Denuvo is specifically designed to detect exactly this kind of debugger attachment.

**User's own call, and the right one**: getting past Denuvo requires genuinely specialized anti-anti-debug tooling (ScyllaHide with Denuvo-current signatures, kernel-level drivers, hypervisor-based analysis, or timing/self-check-neutralization research) that's a different category of undertaking from everything else this project does -- actively defeating a commercial game's anti-piracy DRM, not working with the game's own moddable file formats the way every other technique in this project (pak reading, REFramework, mesh/pfb parsing) does. Correctly identified as out of scope and abandoned; this project continues to rely on community-maintained dumps (REasy) rather than generating its own from scratch.

**Immediate follow-up, same conversation: checked whether REasy's own upstream dump had moved since our last bake (2026-08-10) anyway.** Fetched fresh via the existing `rsz_layout.fetch_latest_dump()` + `install_snapshot(as_role="current", rotate=True)` pipeline (the same safe-merge path #28 built) -- **`corrected: 0, added: 0`, byte-identical field data to what we already had**, confirmed directly by re-testing `fits_current_layout()` on the exact Wyvern Impact VFX donor from item #61: still `False`, unchanged. The upstream REasy dump grew by only 120 bytes since 2026-08-08 (103,929,358 -> 103,929,478) -- consistent with "nothing meaningful changed," not a sampling fluke. Confirms the `via.effect.script.EPVStandardData`/`app.EPVExpertCommonData`/`app.EPVExpertHitData` family gap (item #61's root cause) is a genuine, currently-unresolved limitation of the best publicly available community data, not something a routine re-check fixes -- consistent with this whole family being native/effect-script types, the exact category item #21 already established is chronically under-covered by every available RSZ dump (REasy's own emulation-based guessing can't resolve them either, not just this project's older snapshot). The refresh was still committed (`tools/rsz_fields_mhwilds.json.gz` + a new dated archive entry) since a "checked, confirmed no change" data point has its own small provenance value, matching this project's standing snapshot-archival discipline.

### 64. "적용 방어구 변경" gained pak-based detection -- armor retargeting now supports standalone-.pak mods, mirroring weapon retargeting's existing support (2026-08-16)

Real report: "Shiranui Mai's Outfit" (a real Nexus armor mod, packaged as its own standalone `.pak`) was completely invisible to `slot_retarget.detect_mod_slot()` -- that function only ever scanned for loose-file path patterns (`ch03/<set>/<variant>/...`), and a pak-bundled mod's entries carry no filename at all, only a hash64, so there was nothing to path-match. Weapon retargeting already solved the identical problem for weapons (item #44, `weapon_retarget.detect_mod_weapon_pak()`) -- this item ports that exact technique to armor.

**`slot_retarget.py`**: `ModSlotInfo`/`ModSlotGroup` both gained `pak_path`/`pak_matches` fields, mirroring `weapon_retarget.ModWeaponInfo`'s shape exactly. New `_pak_candidate_hashes(set_no, variant)` computes every plausible pak-entry hash a real armor slot could occupy -- both `ch02`(male)/`ch03`(female) model files (mdf2+mesh) per piece 1-5, both genders' equip pfb (confirmed the male pfb path uses a `ch02`-prefixed filename under `.../armor/male/...`, verified directly against the live game before coding to it), and both genders' `avp.user` -- checked via `pak_path_hash64()` membership against the mod's own pak, the same "if the hash matches a known real path, that unambiguously identifies which slot it is" logic weapon retargeting already established. New `detect_mod_slot_pak()` resolves a single pak to a `ModSlotInfo`; `detect_mod_slot()`/`detect_mod_slots()` both gained a pak-fallback branch, triggered only when zero loose-file matches are found at all -- the existing loose-file code path is completely untouched, so this can't regress any already-working loose-file mod. New `retarget_pak()` rehashes just the identified entries to the target slot's equivalent path (same code/ext/version, only set/variant substituted); every other entry (the mod's own custom-hashed textures) passes through with its original hash64 untouched. `retarget_tree_multi()`/`retarget_archive()` both gained the same `pak_path is not None` branch weapon retargeting already has, reusing the identical "materialize the file list before copying, to avoid a live rglob() re-discovering its own output" safeguard from item #44's own real bug fix.

**GUI**: no changes needed at all -- `_open_retarget_dialog()`'s `do_generate()` already re-extracts fresh from the original archive path and calls `retarget_archive_multi()`, which now transparently handles pak groups; the only GUI-visible difference is a pak-only group showing `0` in the "files" column (cosmetic, `ModSlotGroup.files` stays empty for a pak group since there's nothing to list by path).

**Verified two ways against the real mod**: (1) direct function-level test -- `detect_mod_slot()` correctly resolves `035/001` ("레이아") with 11 pak matches (5 pieces × mdf2+mesh + avp) out of the pak's 34 total entries (the other 23 are the mod's own custom-hashed textures, correctly left unmatched); `retarget_archive()` to a real compatible target (`003/001`) correctly rehashes exactly those 11 entries and leaves the other 23 untouched, output pak entry count unchanged (34), target's new hash confirmed present. (2) full headless GUI test (`TkinterDnD.Tk()` + real `mainloop()`, driving `_open_retarget_dialog()`'s actual code) -- pick, detect, select-candidate, apply, generate all worked end-to-end through the real dialog, including the pre-existing slot-occupancy warning feature correctly firing for other candidates already in use by other installed mods. **Real test-harness bug caught and fixed along the way, not a product bug**: the test's own "wait for output file" step checked bare `Path.exists()`, which goes true the instant `zipfile.ZipFile(..., "w")` creates the file, well before the background worker thread finishes writing entries and closing it -- the test then tore down the whole process (`root.quit()`), killing the daemon worker thread mid-write and producing a genuinely corrupt zip. Fixed by waiting for the file to actually open as a valid zip (`zf.testzip()`) before declaring the test done, not just checking existence -- worth remembering as a general pattern for any future GUI test that polls for a background-thread-produced file.

### 65. Drag-and-drop file support added to both "적용 방어구 변경" and "적용 무기 변경" (2026-08-17)

Real ask: the main window's mod list has supported drag-and-drop (`tkinterdnd2`, `DND_FILES`) since long before this feature set existed, but both retarget dialogs (separate `Toplevel`s, per item #34's own "safer to keep them apart" decision) only ever offered a `filedialog.askopenfilename()` button -- no way to just drop a file onto either window.

**Fix, mechanical, no new dependency**: both dialogs' `do_pick()` (armor: `_open_retarget_dialog()`; weapon: `_open_weapon_retarget_dialog()`) were split into a `start_pick(path)` function (everything that used to run after the file dialog returned a path -- UI reset, background detection thread) and a thin `do_pick()` that just gets a path from `filedialog.askopenfilename()` and calls it. Each dialog's own `win` (the `Toplevel` itself, so dropping anywhere on the window works, not just on the file-entry widget) is registered as a drop target (`win.drop_target_register(DND_FILES)`, guarded by the same `_HAS_DND` flag the main window already checks, so nothing breaks if `tkinterdnd2` is ever unavailable) with a small `on_drop(event)` handler: splits `event.data` via `win.tk.splitlist()` (handles space-containing paths correctly, same as the main window's own `_on_drop`), takes the first dropped path whose suffix is in the existing `ARCHIVE_EXTS` set (`.zip`/`.7z`/`.rar`) and is an actual file (not a directory -- these dialogs work on one archive/extracted-directory at a time, unlike the main window's queue which recurses into dropped folders), and feeds it to `start_pick()`. A drop with no matching archive file is silently ignored, matching the "no file chosen" idle state rather than surfacing a dedicated error.

Verified via a real headless GUI smoke test (`TkinterDnD.Tk()` + `App()`, opening each dialog in turn and confirming no exception from the drop-target registration itself) -- did not simulate an actual OS-level drop event (no tool available to synthesize that), so the underlying `tkinterdnd2` mechanics are trusted as already proven by the main window's own long-standing working use of the identical `DND_FILES`/`dnd_bind("<<Drop>>", ...)` pattern; only the NEW wiring (registration target, path-filtering handler, `start_pick()` refactor) needed independent verification here.

### 66. "적용 방어구 변경"'s slot-occupancy warning (#44) had two real blind spots -- piece 6 skipped entirely, and ch02 never checked at all (2026-08-17)

Real report: the user pointed at a live screenshot of the compatibility candidate list where several already-installed slots (콩가/Congalala 013/001, 투나물/Tunanul 017/001, 푸포루/Puporu 018/001) showed no "already used by another mod" warning at all, while other rows correctly did. Investigated by checking the actual on-disk `natives/` state directly (not just the code) before touching anything, per this project's own established discipline.

**Root cause 1, confirmed via direct filesystem inspection**: `slot_retarget.find_target_occupants()` (added in item #44) iterated a source mod's shipped pieces but unconditionally skipped piece 6 (`if p == 6: continue`), reasoning only about the fact that piece 6 (Slinger) has no `.mdf2` of its own -- true, but it never substituted a check for what piece 6 actually DOES have (`.mesh`), so it couldn't see a piece-6-only occupant at all. Direct on-disk check confirmed exactly this shape at the 3 reported slots: `ch03/013/001/6/ch03_013_0016.mesh...` and `ch03/017/001/6/ch03_017_0016.mesh...` had ONLY a piece-6 mesh file and nothing else -- some other mod (real occupants, resolved via `installed.ini`: "0. Reikyo - Model" and "0. Mangie Himenaios - Model") claimed just a bonus accessory piece there, the exact same "extra piece beyond the standard 5" pattern item #53 already documented for the retargeting-compatibility side, just biting the occupancy-check side instead. 018/001 (푸포루/Puporu, occupant "0. MooMoo - Model") had the same piece-6 mesh PLUS a `textures/` subfolder under piece 2 with no piece-2 mdf2 at the root at all (an "incidental slot" texture stash, item #35's DOTEI-EULA pattern) -- the piece-6 mesh alone was enough to explain the missed warning.

**Root cause 2, found by auditing the rest of the same function against `detect_mod_slots()`'s own established convention**: every path this function built was hardcoded to `ch03` (female) only, even though `detect_mod_slots()`'s own regexes (`_MODEL_SLOT_RE`, `_PIECE_FILE_RE_TMPL`) have matched `ch0[23]` (both playable-character genders) since item #35 specifically because a mod can legitimately ship only one gender's files, or both, independently of the other. No live example was found for this half specifically (Laufey, the mod originally being tested, is ch03-only), but it's the identical asymmetry class as the piece-6 gap, sitting in the same function, so it was fixed in the same pass rather than left for a separate future report.

**Fix**: `find_target_occupants()` now (1) checks every piece the source ships, including 6, using `.mesh` for piece 6 and `.mdf2` for pieces 1-5 -- matching `verify_target_vanilla()`'s own already-established "piece 6 checks mesh (+pfb), not mdf2" convention exactly, rather than inventing a new rule; (2) checks BOTH `ch02` and `ch03` paths per piece (stopping at whichever is found first, since either alone proves the slot is occupied).

Verified directly: re-running the fixed function against the 3 real reported slots now returns the correct occupying files (piece-6 mesh in every case) and `fluffy_installed.find_occupant_names()` correctly resolves them to "Reikyo - Model" / "Mangie Himenaios - Model" / "MooMoo - Model" respectively -- matching what the user expected to see. Re-ran against two ALREADY-correctly-flagged slots (자타/Zata 008/001, 트리스/Tris 009/001) as a regression check -- both still resolve correctly, with piece 6 now ALSO reported as occupied (additional evidence, not a regression) alongside the pieces that were already being caught before this fix. Full headless GUI smoke test (`TkinterDnD.Tk()` + real `mainloop()`) confirms the armor retarget dialog still opens without exception. `weapon_retarget.find_target_occupant()` (the weapon-side sibling) was checked and does NOT have either gap -- weapons have no multi-piece structure at all (a single mdf2 IS the whole slot, no "piece 6" concept), so its existing single-mdf2-glob check was already correct as documented, no change needed there.
