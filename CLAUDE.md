...
**Final coverage: 95/104 unique Korean names now have game-sourced translations** (up from #38's 86 English-only names) in en/ja/zh_tw/zh_cn all at once. The remaining 9 are 4 debug/placeholder entries the source spreadsheet itself doesn't know the meaning of, plus 5 genuine unique/DLC accessory items (검객의 척안(디럭스), 내복?, 도깨비뿔 가발(DLC), 블루밍서클릿, 용인족 귀(DLC)) that `ArmorSeries.msg` doesn't cover at all (a different, not-yet-investigated file would be needed -- likely `Accessory.msg`, per the same `config.toml`'s target list -- left for a future pass since these are a small, cosmetic remainder).
